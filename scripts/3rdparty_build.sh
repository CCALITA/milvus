...
FLEX_OVERRIDE_DIR="${CPP_SRC_DIR}/conan/overrides/flex/2.6.4"
BISON_OVERRIDE_DIR="${CPP_SRC_DIR}/conan/overrides/bison/3.8.2"
LIBTOOL_OVERRIDE_DIR="${CPP_SRC_DIR}/conan/overrides/libtool/2.4.7"
LIBCURL_OVERRIDE_DIR="${CPP_SRC_DIR}/conan/overrides/libcurl/7.86.0"
HWLOC_OVERRIDE_DIR="${CPP_SRC_DIR}/conan/overrides/hwloc/2.9.3"
LIBUNWIND_OVERRIDE_DIR="${CPP_SRC_DIR}/conan/overrides/libunwind/1.8.0"
LIBMOUNT_OVERRIDE_DIR="${CPP_SRC_DIR}/conan/overrides/libmount/2.39.2"
...
# flex/2.6.4 can reach Autotools with configure missing the executable bit on the
# Linux Nix clang+libc++ proof host. Keep using the same public source tarball by exporting
# a tiny local recipe override that restores the mode before autotools.configure().
if [[ "${MILVUS_NIX_CLANG_LIBCXX:-0}" == "1" ]] && [[ -f "${FLEX_OVERRIDE_DIR}/conanfile.py" ]]; then
  echo "Exporting local flex/2.6.4@ override (restore execute bit on configure)"
  run_conan export "${FLEX_OVERRIDE_DIR}" flex/2.6.4@
fi
...
# libcurl/7.86.0's Autotools path can lose libtool's shared data directory under Conan 1
# on the Linux Nix clang+libc++ proof host, which makes libtoolize look for //res/libtool.
# Keep using public sources by exporting a tiny local override that wraps libtoolize with
# the expected _lt_pkgdatadir/aclocal environment before autoreconf.
if [[ "${MILVUS_NIX_CLANG_LIBCXX:-0}" == "1" ]] && [[ -f "${LIBCURL_OVERRIDE_DIR}/conanfile.py" ]]; then
  echo "Exporting local libcurl/7.86.0@ override (wrap libtoolize for Conan 1 Autotools build)"
  run_conan export "${LIBCURL_OVERRIDE_DIR}" libcurl/7.86.0@
fi
# hwloc/2.9.3 can reach Autotools with src/configure missing the executable bit on the
# Linux Nix clang+libc++ proof host. Keep using the same public source tarball by exporting
# a tiny local recipe override that restores the mode before autotools.configure().
if [[ "${MILVUS_NIX_CLANG_LIBCXX:-0}" == "1" ]] && [[ -f "${HWLOC_OVERRIDE_DIR}/conanfile.py" ]]; then
  echo "Exporting local hwloc/2.9.3@ override (restore execute bit on configure)"
  run_conan export "${HWLOC_OVERRIDE_DIR}" hwloc/2.9.3@
fi
# libunwind/1.8.0 can reach Autotools with src/configure missing the executable bit on the
# Linux Nix clang+libc++ proof host. Keep using the same public release tarball by exporting
# a tiny local recipe override that restores the mode before autotools.configure().
if [[ "${MILVUS_NIX_CLANG_LIBCXX:-0}" == "1" ]] && [[ -f "${LIBUNWIND_OVERRIDE_DIR}/conanfile.py" ]]; then
  echo "Exporting local libunwind/1.8.0@ override (restore execute bit on configure)"
  run_conan export "${LIBUNWIND_OVERRIDE_DIR}" libunwind/1.8.0@
fi
# libmount/2.39.2 hits the same missing-executable-bit failure on the Linux Nix clang+libc++
# proof host. Keep using the same public util-linux source tarball by exporting a tiny local
# recipe override that restores src/configure before autotools.configure().
if [[ "${MILVUS_NIX_CLANG_LIBCXX:-0}" == "1" ]] && [[ -f "${LIBMOUNT_OVERRIDE_DIR}/conanfile.py" ]]; then
  echo "Exporting local libmount/2.39.2@ override (restore execute bit on configure)"
  run_conan export "${LIBMOUNT_OVERRIDE_DIR}" libmount/2.39.2@
fi
