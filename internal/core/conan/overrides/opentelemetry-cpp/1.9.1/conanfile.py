...
class OpenTelemetryCppConan(ConanFile):
    ...
    def _nix_build_tool_loader_paths(self):
        ...
        # own package lib dirs during codegen, but the host OpenSSL/libcurl package dirs
        # must stay out so the Nix-shell cmake keeps using the libraries it was linked
        # against.
        dirs = []
        seen = set()
    ...
    def _nix_runtime_loader_paths(self):
        # Preserve the active Nix-shell runtime libraries (clang/libc++/glibc/etc.) while
        # still scrubbing Conan host-package loader pollution. The generated grpc_cpp_plugin
        # binary is built against the Nix toolchain and must retain those /nix/store runtime
        # directories in order to start successfully during protobuf/grpc codegen.
        ...
        for candidate in raw.split(os.pathsep):
            if not candidate or not candidate.startswith("/nix/store/"):
                continue
            if os.path.isdir(candidate) and candidate not in seen:
                seen.add(candidate)
                dirs.append(candidate)
    ...
    def _grpc_cpp_plugin_wrapper(self):
        if not (self._using_nix_clang_libcxx() and self.options.with_otlp_grpc):
            return None

        wrapper = os.path.join(self.build_folder, "grpc_cpp_plugin_wrapper.sh")
        loader_dirs = self._nix_build_tool_loader_paths() + self._nix_runtime_loader_paths()
        quoted_loader_path = os.pathsep.join(loader_dirs)
        conan_user_home = os.environ.get("CONAN_USER_HOME") or os.path.expanduser("~")
    ...
    def _sanitized_loader_env(self):
        ...
        # prepend package OpenSSL/libcurl directories to LD_LIBRARY_PATH. That poisons
        # host-tool startup before opentelemetry-cpp's CMake configure even begins,
        # because the Nix-shell cmake binary then picks up Conan's libcrypto/libcurl
        # instead of the Nix ones it was linked against. Scrub loader env only for the
        # cmake configure/build/install steps so host tools stay in a consistent Nix
        # runtime while dependency resolution still comes from Conan metadata.
        ...
        loader_dirs = self._nix_build_tool_loader_paths() + self._nix_runtime_loader_paths()
        build_tool_loader_path = os.pathsep.join(loader_dirs)
        try:
            os.environ["LD_LIBRARY_PATH"] = build_tool_loader_path
    ...
    def build(self):
        self._patch_sources()
        if self._using_nix_clang_libcxx() and self.options.with_otlp_grpc:
            grpc_cpp_plugin_wrapper = self._grpc_cpp_plugin_wrapper().replace("\\", "/")
            protos_cmake_path = os.path.join(self.source_folder, "cmake", "opentelemetry-proto.cmake")
            replace_in_file(
                self,
                protos_cmake_path,
                ...
                """),
                f'set(gRPC_CPP_PLUGIN_EXECUTABLE "{grpc_cpp_plugin_wrapper}")\n',
            )
        cmake = CMake(self)
