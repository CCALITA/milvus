...
class LibmountConan(ConanFile):
    ...
    def build(self):
        # Conan 1 on the Linux Nix clang+libc++ proof host can unpack util-linux helper
        # scripts without the execute bit. util-linux's autotools regeneration path invokes
        # both the top-level configure script and tools/git-version-gen, so restore the mode
        # in-place before configure rather than mutating the public source tarball.
        for relpath in ("configure", os.path.join("tools", "git-version-gen")):
            candidate = os.path.join(self.source_folder, relpath)
