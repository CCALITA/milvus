...
class HwlocConan(ConanFile):
    ...
    def build(self):
        if self.settings.os == "Windows":
            ...
            # hwloc's generated configure script without the executable bit.
            # The stock recipe executes it directly via autotools.configure(), so
            # restore the mode while keeping the same public source tarball.
